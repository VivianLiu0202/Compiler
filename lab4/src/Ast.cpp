...
{
    std::string type, intvalue,floatvalue;
    type = symbolEntry->getType()->toStr();
    intvalue = symbolEntry->toStr();
    floatvalue = ExprNode::symbolEntry->FloattoStr();
    if(type=="float")
    {
        fprintf(yyout, "%*cFloatLiteral\tvalue: %s\ttype: %s\n", level, ' ',
            floatvalue.c_str(), type.c_str());
    }
    else{
        fprintf(yyout, "%*cIntegerLiteral\tvalue: %s\ttype: %s\n", level, ' ',
            intvalue.c_str(), type.c_str());
    }
